...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Mark only the requested session's close time as result
    const sessionCloseMap: Record<string, string> = {
      "11:00": "11:00",
      "12:01": "12:01",
      "15:00": "15:00",
      "16:30": "16:30",
    };
    const targetClose = sessionCloseMap[requestedOpenKey] || "";
    let resultMarked = false;

    const entries: HistoryEntry[] = allRaw.map(e => {
      const minuteKey = e.time.slice(0, 5);
      let isResult = false;
      if (!resultMarked && targetClose && minuteKey === targetClose) {
        isResult = true;
        resultMarked = true;
      }
    ...
    const resultEntry = entries.find(e => e.isResult);
    const result2d = resultEntry?.twod || "--";
